...
**Researched (v1.5 live metrics addendum):** 2026-02-24
**Confidence:** HIGH (all patterns verified against enterprise and custom source at `/Users/joseph/clones/odoo/`)

---

## v1.5 Scope: Live Metrics (Animated Counters, Token Normalization, Per-Iteration Timing)

**No new npm packages, Python libraries, or Odoo module dependencies are required.** All capabilities needed for v1.5 are achievable with:
- Existing OWL primitives already in the app
- `setRecurringAnimationFrame` from `@web/core/utils/timing` (already in bundle)
- CSS `@keyframes` + `counter()` animation (pure CSS)
- Token fields already present in the `raw_response` payload

The v1.4 subagent, v1.3 persistence, v1.2 theming, and v1.1 OWL standalone stacks are unchanged.

---

## v1.5 Backend: Token Normalization

### Where Token Data Already Lives

Token usage is already present in `raw_response` on every `iteration` bus event. The `iteration` event's `raw_response` field is the full provider API response object passed through `item.get('metadata')` in the `_run_agentic_loop` override (confirmed: `ai_debug/models/ai_session.py` line 206: `'raw_response': item.get('metadata')`; enterprise `ai_session.py` lines 424 and 435 show `yield {'tool_calls': ..., 'metadata': response}` and `yield {'final_message': ..., 'metadata': response}` where `response` is the raw provider return from `get_completions`).

**OpenAI response structure** (from `ai_api_service_openai.py` lines 86-93):
```python
...
response['usage']['output_tokens_details']['reasoning_tokens']  # reasoning portion
```

**Google response structure** (from `ai_api_service_google.py` lines 75-82):
```python
response['usageMetadata']['promptTokenCount']            # prompt tokens
response['usageMetadata']['candidatesTokenCount']        # completion tokens
response['usageMetadata']['cachedContentTokenCount']     # cached portion of input
response['usageMetadata']['thoughtsTokenCount']          # reasoning/thinking portion
```

### Normalized Token Schema

Extract tokens in the Python instrumentation layer (inside `_run_agentic_loop` override) rather than in JavaScript. Python has the raw response before the bus event fires; JS would have to re-parse ambiguous provider formats on every render.

Add a `_ai_debug_normalize_tokens(raw_response)` helper to `ai_debug/models/ai_session.py`:

```python
@staticmethod
def _ai_debug_normalize_tokens(raw_response):
    """Extract token counts from a raw LLM provider response into a common schema.

    Returns a dict with these keys (all integers, 0 if not present):
      - input_tokens:    prompt tokens sent to the model
      - output_tokens:   completion tokens generated by the model
      - cached_tokens:   input tokens served from cache (subset of input_tokens)
      - reasoning_tokens: tokens used for internal reasoning (subset of output_tokens)

    Returns None if raw_response is None or contains no recognized token fields.
    """
    if not raw_response:
        return None

    # OpenAI Responses API format (used by gpt-4o, gpt-4.1, gpt-5, etc.)
    ...
    # Google Gemini format (used by gemini-2.0-flash, gemini-2.5-pro, etc.)
    # response['usageMetadata'] = {'promptTokenCount': N, 'candidatesTokenCount': M,
    #                              'cachedContentTokenCount': K, 'thoughtsTokenCount': R}
    if usage := raw_response.get('usageMetadata'):
        return {
...
Add `token_usage` to the `iteration` bus event payload:
...
self._ai_debug_bus_send('iteration', {
    ...
    # NEW in v1.5: normalized token counts (None if provider returned no usage data)
    'token_usage': self._ai_debug_normalize_tokens(item.get('metadata')),
})
```

Also add a `total_tokens` rollup to `loop_end` for trace-level aggregate display:

```python
# In _run_agentic_loop override: accumulate token counts across iterations
_total_input_tokens = 0
_total_output_tokens = 0

# Inside the iteration event block, after normalization:
if token_usage := self._ai_debug_normalize_tokens(item.get('metadata')):
    _total_input_tokens += token_usage['input_tokens']
    _total_output_tokens += token_usage['output_tokens']
...
self._ai_debug_bus_send('loop_end', {
    ...
    # NEW in v1.5: trace-level token totals
    'total_input_tokens': _total_input_tokens,
    'total_output_tokens': _total_output_tokens,
})
```

**Why accumulate in Python rather than sum in JS:** The JS already receives `iteration` events and could sum `token_usage` fields. However, the `loop_end` event fires in the `finally` block — accumulating there means the sidebar can display the total immediately on trace completion without a JS reduce pass over all iterations. Both approaches work; Python accumulation is marginally simpler at display time.

---

## v1.5 Backend: Per-Iteration Timing

### Current State

`time.monotonic()` is already called at loop start (`started_at = time.monotonic()`) and the `duration_ms` on `loop_end` gives total trace wall time. Per-iteration timing does not exist.

### What to Add

Track iteration start times using `time.monotonic()` at the start of each iteration:

```python
# In _run_agentic_loop override, inside the for loop over super():
iteration_start = time.monotonic()  # capture BEFORE the iteration event fires

# ... build iteration_id, messages_snapshot, token_usage ...

self._ai_debug_bus_send('iteration', {
    ...
    # NEW in v1.5: wall-clock duration from previous iteration end (or loop start) to now
    # This is the LLM API latency for this iteration.
    # "How long did it take the LLM to respond?"
    'duration_ms': int((time.monotonic() - iteration_start) * 1000),
    'token_usage': ...,
})
```

**What the timer measures:** The time from when `super()._run_agentic_loop` is entered for this iteration (i.e., the LLM API call is issued) to when the response is received and processed. This is the LLM API latency — the most useful per-iteration timing for a developer.

**Implementation note:** `iteration_start` should be captured immediately before the `for item in super()...` loop body executes for that iteration. In the current override structure, the `for` loop yields one item per LLM response. The start time for iteration N should be captured after tool calls from iteration N-1 finish (i.e., the next iteration of the for loop begins). The cleanest approach:

```python
iteration_start = time.monotonic()  # loop start

for item in super()._run_agentic_loop(...):
    if 'tool_calls' in item or 'final_message' in item:
        iteration_duration_ms = int((time.monotonic() - iteration_start) * 1000)
        iteration_count += 1
        # ... emit iteration event with duration_ms=iteration_duration_ms ...
        iteration_start = time.monotonic()  # reset for next iteration
    yield item
```

This correctly measures: loop_start → first_LLM_response (iteration 1 duration), then first_tool_call_end → second_LLM_response (iteration 2 duration), etc. Tool call execution time IS included in iteration N+1's duration (it's the time between responses), which is semantically correct: "how long was the agent thinking/acting between LLM responses?"

---

## v1.5 Frontend: Token Data in Reactive Store

### Store Changes

The `iteration` store entry gains two new fields when `_onIteration` processes a bus event:
...
this._onIteration = (payload) => {
    ...
    trace.iterations.set(payload.iteration_id, {
        // ...all existing fields...
        // NEW in v1.5:
        token_usage: payload.token_usage || null,   // {input_tokens, output_tokens, cached_tokens, reasoning_tokens} or null
        duration_ms: payload.duration_ms || null,   // LLM API latency for this iteration in ms
    });
};
```

The `trace` root entry gains token totals from `loop_end`:

```javascript
this._onLoopEnd = (payload) => {
    const trace = this.traces.get(payload.trace_id);
    if (!trace) return;
    trace.status = /* ...existing status mapping... */;
    trace.ended_at = new Date();
    trace.duration_ms = payload.duration_ms;
    // NEW in v1.5:
    trace.total_input_tokens = payload.total_input_tokens || 0;
    trace.total_output_tokens = payload.total_output_tokens || 0;
    // ...existing IDB write...
};
```

**IDB impact:** The existing `serializeTrace()` function will automatically include these new fields in IDB writes because it uses the spread `...iter` pattern for iteration fields (confirmed in `db.js`). The `hydrateTrace()` function reconstructs from the stored object, so new fields pass through transparently. No `DB_VERSION` bump required — additive fields do not break existing stored records.

---

## v1.5 Frontend: Animated Counter Display

### What "Counting Up" Means Concretely

The sidebar trace row shows a compact metric chip like `1.2s · 847 tok`. As new iterations arrive, the number animates upward to its new value (e.g., from 412 to 847). The animation runs for ~600ms after each new iteration event. This is a UX affordance signaling "something just updated" — not a real-time ticker.

**Two distinct display needs:**

| Display | Where | Behavior | Animation |
|---------|-------|----------|-----------|
| Sidebar compact chip | Trace row | Total time elapsed + total tokens | Count-up on each new iteration |
| Detail panel elapsed timer | Trace header | Live seconds ticker while running | Continuous tick using rAF |
| Detail panel per-iteration breakdown | Iteration tab | Static numbers per iteration | No animation (static display) |
| Detail panel trace totals | Summary section | Total tokens, total time | No animation (static display) |

### Pattern 1: Count-Up Animation (Sidebar Chips)

The count-up animation for token and time totals in sidebar rows uses pure CSS with a `@property` custom property and a `@keyframes` animation on a numeric CSS counter. This requires no JavaScript state mutation on each frame.

**CSS-only approach using `@property` and `counter()`:**

```scss
// Define an animatable integer CSS custom property
@property --ai-counter-val {
    syntax: '<integer>';
    initial-value: 0;
    inherits: false;
}

.ai-metric-chip-animated {
    animation: ai-count-up 0.6s ease-out;
    counter-reset: ai-metric var(--ai-counter-val);

    // Display the counter value as text
    &::after {
        content: counter(ai-metric);
    }
}

@keyframes ai-count-up {
    from { --ai-counter-val: 0; }  // starts from 0
    to   { --ai-counter-val: var(--ai-counter-target); }  // ends at target value
}
```

**Browser support:** `@property` (Houdini CSS Properties and Values API) is supported in Chrome 85+, Edge 85+, Safari 16.4+, Firefox 128+. Coverage is sufficient for a developer tool targeting modern browsers (February 2026). LOW-MEDIUM confidence on Firefox: Firefox only added `@property` support in version 128 (2024), confirm with `@supports` guard.

**Limitation:** CSS `counter()` only works in `content` — it cannot set the `innerText` of a span. The animated value can only appear as a CSS-generated `::after` pseudo-element. This limits formatting options (e.g., cannot insert "tok" label between the number and unit).

**Fallback: JavaScript rAF count-up (recommended over CSS for complex formatting):**

Use `requestAnimationFrame` directly in an OWL component for the count-up. The component receives the target value as a prop, maintains a `useState` counter, and ticks it up over 600ms:
...
/**
 * Animated number display — counts from current displayed value up to `target`.
 * Uses requestAnimationFrame for smooth animation without OWL re-render spam.
 *
 * Props:
 *   target: number — the value to animate to
 *   duration: number — animation duration in ms (default 600)
 *   suffix: string — text appended after the number (e.g., ' tok', 'ms')
 */
export class AnimatedCounter extends Component {
    static template = "ai_debug.AnimatedCounter";
    static props = {
        target: { type: Number },
    ...
    setup() {
        this.state = useState({ displayed: 0 });
        ...
        useEffect(
            (target, duration) => {
                ...
                this._startValue = this.state.displayed;
                this._startTime = null;
                ...
                const tick = (timestamp) => {
                    ...
                    this.state.displayed = Math.round(
                        this._startValue + (target - this._startValue) * eased
                    );
                    if (progress < 1) {
                ...
                return () => {
                    // Cleanup: cancel animation on prop change or unmount
                    if (this._rafHandle !== null) {
                        cancelAnimationFrame(this._rafHandle);
                        this._rafHandle = null;
                    }
            ...
            () => [this.props.target, this.props.duration]
        );
    }
}
```

**OWL reactivity concern:** Each `this.state.displayed = ...` in the rAF tick triggers an OWL re-render of `AnimatedCounter` (and its parent, due to OWL's top-down re-render). For a ~600ms animation at 60fps, this is ~36 re-renders. The component tree is shallow (sidebar row → metric chip → counter). This is acceptable for a dev tool; profile if jank appears.

**Alternative: mutate DOM directly, bypass OWL.** Use `useRef` on a `<span>` and update `el.textContent` directly in the rAF tick. Zero OWL re-renders. The tradeoff is departing from OWL's declarative model for this one component:

```javascript
// In AnimatedCounter setup():
this.spanRef = useRef("value");
...
useEffect(
    (target, duration) => {
        ...
        const tick = (timestamp) => {
            // ...
            if (this.spanRef.el) {
                this.spanRef.el.textContent = Math.round(...).toLocaleString();
            }
            // ...
        };
    },
...
**Recommendation: use the `useRef` + direct DOM mutation approach.** It produces visually identical results with zero re-render overhead. The OWL reactive approach (option 1) works but produces unnecessary parent re-renders — wasteful for a purely visual animation that changes no application state.

### Pattern 2: Live Elapsed Time Ticker (Detail Panel Header)

The detail panel for a running trace should show "Running for 3.2s" with the number updating every second. This is a continuous live ticker, not a count-up animation.

Use `setRecurringAnimationFrame` from `@web/core/utils/timing` (verified present in Odoo master at `addons/web/static/src/core/utils/timing.js` lines 101-116). It calls a callback repeatedly via `requestAnimationFrame`, passing the delta time:
...
// In LoopDetail or trace detail component:
setup() {
    this.state = useState({ elapsedMs: 0 });
    ...
    onWillUnmount(() => {
        if (this._stopTicker) this._stopTicker();
    });
}

_startTicker() {
    const startTime = Date.now() - (/* already elapsed */ 0);
    this._stopTicker = setRecurringAnimationFrame(() => {
        this.state.elapsedMs = Date.now() - startTime;
    });
}
```

**Why `setRecurringAnimationFrame` and not `setInterval`:** rAF pauses automatically when the browser tab is backgrounded, preventing unnecessary computation. `setInterval` fires regardless of tab visibility. For a dev tool, rAF is the correct choice. `setRecurringAnimationFrame` from Odoo's timing utils handles the cleanup pattern and is already in the bundle.

**Granularity:** Update the displayed elapsed time no faster than once per second to avoid visual noise. Debounce the `state.elapsedMs` update to 1s buckets:

```javascript
this._stopTicker = setRecurringAnimationFrame((delta) => {
    // delta is time since last frame in ms; accumulate until 1s passes
    this._accumulated = (this._accumulated || 0) + delta;
    if (this._accumulated >= 1000) {
        this._accumulated -= 1000;
        this.state.elapsedMs = Date.now() - this._traceStartTime;
    }
});
```

**Alternative: `setInterval` at 1s.** Simpler code. For a dev tool tab that is actively watched, the tab-backgrounding concern is irrelevant. Both approaches work; `setInterval` is slightly simpler if the rAF-per-frame pattern feels overengineered for 1s granularity.

### Pattern 3: Sidebar Compact Metric Chips

The sidebar trace row shows compact metrics inline with the trace label. Design constraints: row height is 44px (fixed by CSS), available width is narrow (after label and status icon). Metrics must be visually subordinate to the trace label.

**Recommended display format:**

```
[trace label truncated]          [metric chip]  [status]
Query: "Create leads for..."     1.2s · 847↑    ✓
```

The metric chip contains:
- Total wall time: formatted via existing `_formatDuration(trace.duration_ms)` method
- Total tokens: input+output sum, formatted with `k` suffix for ≥1000 (e.g., `1.2k`)
- A small upward arrow or "↑" suffix on tokens to indicate output direction (optional)

```javascript
// In app.js — add to AiDebugApp:
formatTokenCount(n) {
    if (!n) return null;
...
// In template — on trace rows:
// Show when trace is running (no duration_ms yet) — live elapsed approximation
// Show when trace is complete — from trace.duration_ms and trace.total_tokens
```

**For running traces:** Show a pulsing "..." or the running elapsed time from `started_at`. The live elapsed time ticker (Pattern 2) is in the detail panel; the sidebar row can show a simpler "running" indicator (existing pulsing dot already does this).

**For completed traces:** Show `duration_ms` formatted + total tokens. No animation needed — the trace is done.

**For "just completed" traces (animation trigger):** When `_onLoopEnd` fires, briefly animate the token count chip from the prior value to the final value. Trigger by setting a `_justCompletedId` flag and clearing it after the animation duration.

Implementation approach: use the `useRef` + direct DOM mutation pattern from Pattern 1. No need for a dedicated `AnimatedCounter` component — a simple `onPatched` callback in `AiDebugApp` can find the chip element by `data-trace-id` and trigger the CSS animation class:

```javascript
onPatched(() => {
    // ...existing scroll/flash logic...
    // Trigger count-up animation on newly completed trace chips
    if (this._animateChipId && this.sidebarRef.el) {
        const el = this.sidebarRef.el.querySelector(
            `[data-chip-id="${this._animateChipId}"]`
        );
        if (el) {
            el.classList.remove("ai-chip-animate");
            // Force reflow to restart animation
            void el.offsetWidth;
            el.classList.add("ai-chip-animate");
            setTimeout(() => el.classList.remove("ai-chip-animate"), 700);
        }
        this._animateChipId = null;
    }
});
```

This approach reuses the proven `onPatched` + `classList` pattern already used for the `ai-tree-flash` animation.

---

## v1.5 Frontend: Integration Points

### Where NOT to Use `setRecurringAnimationFrame` or `setInterval`

Do not start a live ticker inside the sidebar `sidebarNodes` computed getter or any computed property called during render. The getter is called synchronously during OWL's render pass — starting async work there causes render-in-render issues. Always start tickers in lifecycle hooks (`onMounted`, `onWillStart`) or event handlers.

### OWL `useEffect` for Animation Lifecycle

The `useEffect` hook (from `@odoo/owl`) is the correct way to run side effects that depend on prop values in OWL 2:
...
useEffect(
    (target) => {
        ...
        const stop = startAnimation(target);
        return stop;  // cleanup function — called before next effect run or unmount
    },
    () => [this.props.target]  // dependency array as a getter
);
```

`useEffect` in OWL 2 is analogous to React's `useEffect` but is stable in Odoo master (HIGH confidence — used throughout Odoo core OWL components). The cleanup return value is called before the next effect invocation, making it safe for rAF/timer cleanup.

### Avoiding Re-Render Storms During Animation

The risk: a `useState` field updated 60 times/second triggers 60 OWL re-renders per second. For a dev tool with a complex sidebar tree (dozens of nodes), this can cause jank.

**Rule:** Never write animation frames to `useState`. Write to DOM directly via `useRef`.

```javascript
// BAD — triggers OWL re-render on every frame
this.state.displayedTokens = newValue;  // in rAF callback

// GOOD — bypasses OWL, directly mutates DOM
if (this.chipRef.el) this.chipRef.el.textContent = newValue + " tok";
```

This is a departure from OWL's declarative model but is the correct approach for animations. OWL's reactive system is for application state, not animation frames.

---

## v1.5 What NOT to Use

| Avoid | Why | Use Instead |
|-------|-----|-------------|
| External counter animation libraries (countUp.js, animate.js) | External npm dependency not in Odoo's asset pipeline; adds weight for a feature implementable in 20 lines of JS. | `requestAnimationFrame` + cubic easing in a `useRef` pattern |
| `CSS @property` + `counter()` for animated numbers | Browser support gap (Firefox < 128); limited to `::after` pseudo-element content only; cannot format numbers with commas/suffixes inline with other text. | rAF + direct DOM mutation via `useRef` |
| `setInterval` for the live elapsed ticker | Fires when tab is backgrounded; marginally worse than rAF. | `setRecurringAnimationFrame` from `@web/core/utils/timing` |
| Updating `useState` on every rAF frame | Triggers OWL re-renders at 60fps; sidebar with dozens of nodes causes visible jank. | Direct DOM mutation via `useRef` for animation values only |
| Extracting token counts in JavaScript from `raw_response` | Each JS render would re-parse provider-specific formats (OpenAI `usage` vs Google `usageMetadata`). Fragile to provider API changes. | Python normalization at bus-send time; JS receives `token_usage` already normalized |
| Adding Anthropic/Claude token normalization now | Only OpenAI and Google are active in the enterprise `ai` module. Anthropic format is not present in source. | Add only when Anthropic provider is added to enterprise module |
| Per-frame `this.state.elapsedMs` updates for the live ticker | 60fps state updates with OWL re-renders. | 1-second granularity; update state once per second, not per frame |
| `webclient_rendering_context()` — wrong API name | The correct method is `color_scheme()` on `ir.http`. The `webclient_rendering_context()` pattern shown in PROJECT.md is what the controller already uses; no change needed. | `request.env['ir.http'].color_scheme()` (already implemented) |

---

## v1.5 Confidence Assessment

| Area | Confidence | Basis |
|------|------------|-------|
| OpenAI token field locations | HIGH | Direct source read: `ai_api_service_openai.py` lines 86-93 |
| Google token field locations | HIGH | Direct source read: `ai_api_service_google.py` lines 75-82 |
| `raw_response` carries the full provider response | HIGH | `ai_debug/models/ai_session.py` line 206: `'raw_response': item.get('metadata')`; enterprise `ai_session.py` lines 424, 435 confirm `metadata` = raw `response` from `get_completions` |
| `setRecurringAnimationFrame` in `@web/core/utils/timing` | HIGH | Direct source read: `addons/web/static/src/core/utils/timing.js` lines 101-116 |
| `useEffect` in OWL 2 | HIGH | Used throughout Odoo core OWL components in master |
| `useRef` + direct DOM mutation for animation | HIGH | Same pattern as existing `sidebarRef`, `selectAllRef`, `fileInputRef` in app.js; `onPatched` DOM manipulation already in use for flash animations |
| IDB additive field pass-through (no version bump) | HIGH | Existing `serializeTrace` uses spread operators; new fields on iteration objects serialize automatically |
| `@property` CSS animation | MEDIUM | Supported Chrome 85+, Safari 16.4+, Firefox 128+. Firefox 128 released July 2024 — most developer machines running recent Firefox will have it. But cannot guarantee. Use `@supports` guard or JS fallback. |
| rAF count-up OWL re-render cost | MEDIUM | Known OWL characteristic; not empirically measured in this specific sidebar. Profile if jank appears in practice. |
...
- `/Users/joseph/clones/odoo/custom/.worktrees/master-ai-sub-agents-dpro/ai_debug/models/ai_session.py` lines 199-208 — `iteration` event payload including `raw_response: item.get('metadata')` (HIGH confidence)
- `/Users/joseph/clones/odoo/enterprise/.worktrees/master-ai-sub-agents-dpro/ai/models/ai_session.py` lines 422-435 — `_run_agentic_loop` yielding `{'tool_calls': ..., 'metadata': response}` and `{'final_message': ..., 'metadata': response}` where `response` is the raw provider return (HIGH confidence)
- `/Users/joseph/clones/odoo/enterprise/.worktrees/master-ai-sub-agents-dpro/ai/services/ai_api_service_openai.py` lines 78-100 — OpenAI `usage` field structure with `input_tokens`, `output_tokens`, `input_tokens_details`, `output_tokens_details` (HIGH confidence)
- `/Users/joseph/clones/odoo/enterprise/.worktrees/master-ai-sub-agents-dpro/ai/services/ai_api_service_google.py` lines 66-83 — Google `usageMetadata` field structure with `promptTokenCount`, `candidatesTokenCount`, `cachedContentTokenCount`, `thoughtsTokenCount` (HIGH confidence)
- `/Users/joseph/clones/odoo/odoo/.worktrees/master/addons/web/static/src/core/utils/timing.js` lines 101-116 — `setRecurringAnimationFrame` implementation (HIGH confidence)
- `/Users/joseph/clones/odoo/custom/.worktrees/master-ai-sub-agents-dpro/ai_debug/static/src/app/app.js` lines 342-366 — existing `onPatched` pattern with `classList` animation and `useRef` DOM manipulation (HIGH confidence)
- `/Users/joseph/clones/odoo/custom/.worktrees/master-ai-sub-agents-dpro/ai_debug/static/src/app/app.scss` lines 529-537 — existing `@keyframes ai-tree-flash` CSS animation pattern (HIGH confidence)
- `/Users/joseph/clones/odoo/custom/.worktrees/master-ai-sub-agents-dpro/ai_debug/static/src/app/app.js` lines 728-759 — existing `getIterationDuration` and `_formatDuration` helpers (already computes time between iterations from `receivedAt` timestamps) (HIGH confidence)

---

## v1.5 What IS Already Done (Do Not Re-Research)

- Per-iteration timing approximation via `receivedAt` timestamps: `getIterationDuration()` in `app.js` already computes approximate iteration duration from the gap between consecutive iteration `receivedAt` times. v1.5 replaces this approximation with the authoritative server-measured `duration_ms` from the `iteration` event. The existing method can be kept as fallback for hydrated historical traces that lack `duration_ms`.
- Total trace duration: already in `trace.duration_ms` from the `loop_end` event.
- Running trace visual indicator: the pulsing dot `ai-debug-pulse-dot.small` already appears on running traces.
- `_formatDuration` helper: already formats ms into "1.2s", "2m 3s" etc. — reuse for both trace and iteration duration display.
...
| `setRecurringAnimationFrame` | Odoo master | Exported from `@web/core/utils/timing`. Calls callback via `requestAnimationFrame` repeatedly until the returned stop function is called. |
| `useEffect` | OWL 2 (Odoo master) | Side-effect hook with dependency array and cleanup. Import from `@odoo/owl`. |
...
- `addons/web/static/src/core/utils/timing.js` lines 101-116 — `setRecurringAnimationFrame` full implementation (HIGH confidence)
...
| `web.dark_mode_variables` bundle | web_enterprise master | Defined in `web_enterprise/__manifest__.py` lines 44-75. Required for the `ai_debug.assets_dark` approach. |
...
*Stack research for: AI Debugger — live metrics (animated counters, token normalization, per-iteration timing) (v1.5)*
